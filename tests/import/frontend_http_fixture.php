...
$ins->execute(['EC3', "Emperor's Children ABOVE window", 'https://s.example/p/ec3', 9.99]); // ueber missing+2
...
// 20 weitere in-window Produkte fuer Pagination (insgesamt 26 Treffer):
$insP = $pdo->prepare('INSERT INTO VSKR_products
    (shop_id, external_id, name, url, price, available, category, last_seen_at, updated_at)
    VALUES (1, ?, ?, ?, ?, 1, NULL, NOW(), NOW())');
$insCatP = $pdo->prepare('INSERT IGNORE INTO VSKR_product_categories (shop_id, external_id, category) VALUES (1, ?, ?)');
for ($i = 1; $i <= 20; $i++) {
